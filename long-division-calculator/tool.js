...
function startSolver() {
    ...
    // Note: The HTML uses name="mode", so this querySelector works perfectly
...
function updateNavButtons() {
    ...
    // Note: Removed dark-mode logic for now as it relied on body class, 
    // but basic highlight logic remains.
    const box = document.querySelector('.step-explanation-box');
    if (!box) return;

    if (steps[currentStepIndex] && steps[currentStepIndex].isDone) {
        // Highlight green on completion
